#!/usr/bin/env bash
#
# pred_eval_overlay_3cls_merge.sh
#   merge 3cls 가중치로 big/little/tomatod test set 에 대해
...
#
# 결과 경로:
#   result/detection_only/merge/<target>/<model>_<target>_prediction
#   result/detection_only/merge/<target>/<model>_<target>_eval
#
# 예시:
#   BENCH_GPU=2 ./scripts/pred_eval_overlay_3cls_merge.sh
#   BENCH_GPU=2 ./scripts/pred_eval_overlay_3cls_merge.sh --models "yolo11_3cls rf_detr_3cls"
#   ./scripts/pred_eval_overlay_3cls_merge.sh --no-predict --no-eval
#   ./scripts/pred_eval_overlay_3cls_merge.sh --show-conf --max-images 50
...
if ! "$PY" -c "import yaml, torch, rfdetr, pandas; from PIL import Image" >/dev/null 2>&1; then
  echo "[pred_eval_overlay_3cls_merge][경고] '$PY' 에 필요한 패키지(yaml/torch/rfdetr/pandas/PIL)가 없습니다."
  echo "  올바른 venv 로 다시 실행하세요. 예:"
  echo "    PYBIN=/home/hyeonjin/detect-and-reason/.venv/bin/python $0"
  exit 1
fi

TMP_DIR="$(mktemp -d /tmp/pred_eval_overlay_3cls_merge.XXXXXX)"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
echo "[pred_eval_overlay_3cls_merge] split=${SPLIT} gpu=${GPU:-unset} py=${PY}"
echo "[pred_eval_overlay_3cls_merge] predict=${DO_PREDICT} eval=${DO_EVAL} overlay=${DO_OVERLAY} show_conf=${SHOW_CONF}"
echo "[pred_eval_overlay_3cls_merge] models=${MODELS}"
echo "[pred_eval_overlay_3cls_merge] train_dataset=${TRAIN_DATASET} targets=${TARGET_DATASETS[*]}"
...
for eval_dataset in "${TARGET_DATASETS[@]}"; do
  for model in $MODELS; do
    ...
    if [[ "$DO_EVAL" -eq 1 ]]; then
      ...
      if [[ "$family" == "rf_detr" ]]; then
        local_coco_split="$(coco_split_dir_name "$SPLIT")"
        "$PY" scripts/evaluate_coco_predictions.py \
          --model-config "$model_cfg" \
          --dataset "$eval_dataset" \
          --split "$SPLIT" \
          --pred-coco "$pred_dir/${local_coco_split}/predictions_coco.json" \
          --output-dir "$eval_dir" \
...
echo "=================================================================="
echo "[pred_eval_overlay_3cls_merge] 완료"
